---
title: Prise en main
sidebarTitle: Prise en main
---

import { Button } from '/snippets/fr/button.mdx';

<div id="getting-started-with-the-batch-compliance-endpoints">
  ## Prise en main des endpoints de conformité par lots
</div>

L’utilisation des endpoints de conformité par lots implique généralement 5 étapes :

1. Création d’un job de conformité
2. Préparation de la liste d’ID de Post ou d’ID utilisateur
3. Téléchargement de la liste d’ID de Post ou d’ID utilisateur
4. Vérification de l’état du job de conformité
5. Téléchargement des résultats

Dans cette section, nous allons apprendre à réaliser chacune de ces étapes en utilisant la ligne de commande. Si vous souhaitez consulter des exemples de code dans différents langages de programmation, veuillez visiter notre [dépôt GitHub d’exemples pour X API v2](https://github.com/xdevplatform/Twitter-API-v2-sample-code).

<div id="prerequisites">
  ### Prérequis
</div>

Pour suivre ce guide, vous aurez besoin d’un jeu de [clés et jetons](/fr/resources/fundamentals/authentication) pour authentifier votre requête. Vous pouvez générer ces clés et jetons en suivant ces étapes :

* [Créez un compte développeur](https://developer.x.com/en/apply-for-access) et obtenez son approbation.
* Créez un [projet](/fr/resources/fundamentals/projects) et une [application développeur](/fr/resources/fundamentals/developer-apps) associée dans le portail développeur.
* Accédez à la page « Keys and tokens » de votre application pour générer les identifiants requis. Veillez à conserver tous les identifiants dans un emplacement sécurisé.

<div id="step-one-create-a-compliance-job">
  #### Première étape : créer une tâche de conformité
</div>

Commencez par créer une tâche de conformité et indiquez si vous allez importer des identifiants de Posts ou des identifiants d’utilisateurs (à l’aide du paramètre type). Vous pouvez également nommer votre tâche (à l’aide du paramètre name).

Pour autoriser cette requête, utilisez l’authentification [App only](/fr/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token). Pour ce faire, remplacez le $APP_ACCESS_TOKEN ci‑dessous par votre [App Access Token](/fr/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token), que vous pouvez générer dans votre [X App](/fr/resources/fundamentals/developer-apps) sur le portail développeurs.

`curl --request POST 'https://api.x.com/2/compliance/jobs' \ --header 'Authorization: Bearer $APP_ACCESS_TOKEN' \ --header 'Content-Type: application/json' \ --data-raw '{"type": "tweets"}'`

Si votre appel à l’API aboutit, vous recevrez une réponse similaire à la suivante :

```
{
   "data": {
       "download_expires_at": "2021-08-18T19:42:55.000Z",
       "status": "created",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/submission/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "created_at": "2021-08-11T19:42:55.000Z",
       "resumable": false,
       "id": "1425543269983784962",
       "type": "tweets",
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/delivery/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "upload_expires_at": "2021-08-11T19:57:55.000Z"
   }
}
```

Notez la valeur des champs upload_url, download_url et id ; vous en aurez besoin dans les étapes suivantes.

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### Deuxième étape : préparer une liste d’ID de Post ou d’ID d’utilisateur
</div>

Créez un fichier texte contenant des ID de Post ou des ID d’utilisateur, avec un ID de Post ou un ID d’utilisateur par ligne. Le contenu du fichier texte peut ressembler à ceci :

1417856744319971329

1415457498803232770

1415348607813832708

1413515358766452738

. . .

. . .

<Note>
  **Remarque** : Le fichier ci-dessus peut contenir soit des ID de Post, soit des ID d’utilisateur, mais pas un mélange des deux.

  Vous pouvez téléverser votre fichier directement vers cette URL via une requête **PUT**. Notez que l’URL est déjà signée avec un jeton d’authentification, ce qui signifie que vous n’aurez pas à vous authentifier de nouveau en transmettant votre App Access Token. Assurez-vous d’inclure un en-tête Content-Type pour indiquer que vous téléversez un fichier texte et remplacez $FILE_LOCATION ci-dessous par le chemin de votre fichier.
</Note>

```
curl -X PUT  \
 -H "Content-Type: text/plain" \
 --data-binary @$FILE_LOCATION \
"https://storage.googleapis.com/twitter-compliance/customer_test_object_123456_d8ske9.json?X-Goog-Algorithm=\nGOOG4-RSA-SHA256&X-Goog-Credential=example%40example-project.iam.gserviceaccount\n.com%2F20181026%2Fus-central-1%2Fstorage%2Fgoog4_request&X-Goog-Date=20181026T18\n1309Z&X-Goog-Expires=900&X-Goog-SignedHeaders=host&X-Goog-Signature=247a2aa45f16\\n2ea7abedc098d2eb14a7"
```

Un code d’état 200 indiquera que le téléversement a réussi.

<div id="step-three-optional-check-your-job-status">
  #### Troisième étape (facultative) : Vérifier l’état de votre tâche
</div>

Les envois volumineux peuvent prendre du temps à être traités. Vous pouvez demander une mise à jour de l’état via l’endpoint du job de conformité du contenu en indiquant l’ID du job reçu lors de la première étape.

Encore une fois, veillez à remplacer ci‑dessous $APP_ACCESS_TOKEN par votre App Access Token, et $ID par l’ID de votre job de la première étape.

`curl --request GET 'https://api.x.com/2/compliance/jobs/$ID' \ --header 'Authorization: Bearer $APP_ACCESS_TOKEN'`

La réponse inclura des informations sur la tâche.

```
{
   "data": {
       "upload_expires_at": "2021-08-05T01:50:11.000Z",
       "type": "tweets",
       "resumable": false,
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1423095206576984067/delivery/1202726487847104512_1423095206576984067?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210805%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210805T013511Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "id": "1423095206576984067",
       "status": "expired",
       "created_at": "2021-08-05T01:35:11.000Z",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1423095206576984067/submission/1202726487847104512_1423095206576984067?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210805%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210805T013511Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "download_expires_at": "2021-08-12T01:35:11.000Z"
   }
}
```

Un statut complete signifie que les résultats sont prêts à être téléchargés. Remarque : les autres valeurs de statut peuvent être created, complete, in_progress, failed et expired

<div id="step-four-download-the-results">
  #### Étape quatre : Télécharger les résultats
</div>

Lorsque vous recevez l’état d’achèvement du job, vous pouvez télécharger les résultats de conformité à partir de l’URL indiquée dans le champ download_url (également générée à la première étape). Cette URL est déjà signée avec un jeton d’authentification, ce qui signifie que vous n’aurez pas besoin de vous authentifier en transmettant à nouveau votre App Access Token.

```
curl --request GET \
'https://storage.googleapis.com/twttr-tweet-compliance/1423047488781488129/delivery/1202726487847104512_1423047488781488129?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210804%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210804T222534Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature='
```

Le résultat contiendra un ensemble d’objets JSON (un objet par ligne). Chaque objet contiendra un ID de Post ou d’utilisateur, la date de création du Post ou de l’utilisateur (utile pour retrouver les Posts classés par date), l’action requise, la raison de l’action de conformité et sa date :

```
{"id":"1265324480517361664","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"deleted"}
{"id":"1263926741774581761","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"protected"}
{"id":"1265324480517361669","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"suspended"}
```

Votre code peut analyser chaque ligne JSON pour repérer l’ID du Post ou de l’utilisateur et supprimer de votre jeu de données les Posts et utilisateurs portant ces IDs afin de rester conforme. S’il n’existe pas d’objet JSON correspondant pour un ID que vous avez envoyé, vous pouvez supposer que cet ID est conforme.

**Remarque** : Tous les événements de conformité ne comporteront pas le champ redacted_at.