---
title: Guide d’intégration
sidebarTitle: Guide d’intégration
---

import { Button } from '/snippets/fr/button.mdx';

<div id="working-with-resumable-uploads">
  ## Utiliser les téléversements résumables
</div>

Avec les endpoints de conformité Batch, les développeurs peuvent importer par lots de grands volumes de données X et déterminer les actions nécessaires pour que leurs jeux de données reflètent l’intention des utilisateurs et l’état actuel du contenu sur X. L’envoi de gros volumes de données vers un serveur distant est relativement simple lorsque les systèmes et la connectivité sont stables et fiables. Toutefois, ce n’est pas toujours le cas. Certains environnements peuvent imposer un délai d’expiration, interrompant la connexion entre votre application et le serveur d’envoi après un certain temps ; vous pouvez également rencontrer des problèmes de connexion, par exemple lorsque vous tentez d’importer un fichier volumineux depuis votre ordinateur portable via une connexion Wi‑Fi. Dans ces situations, il est préférable de téléverser le fichier par petites portions, plutôt que d’établir une seule connexion continue.

Les endpoints de conformité par lots de X s’appuient sur Google Cloud Storage pour traiter de gros fichiers. Ce type de stockage est optimisé pour divers usages ; Cloud Storage prend en charge une technique de gestion des gros fichiers appelée téléversement résumable.

Si l’envoi échoue à un moment donné, Google Cloud Storage peut reprendre l’opération là où elle s’est arrêtée.

<div id="creating-a-resumable-job">
  ### Créer une tâche réutilisable
</div>

<div id="step-one">
  #### Première étape :
</div>

Commencez par créer une tâche de conformité et indiquez si vous allez envoyer des IDs de Post ou des IDs d’utilisateur (à l’aide du paramètre type). Ajoutez également resumable dans le corps et définissez-le sur true. Veillez à remplacer $APP&#95;ACCESS&#95;TOKEN ci-dessous par votre jeton d’accès « App only ».

```
curl --request POST \
 'https://api.x.com/2/compliance/jobs' --header 'Authorization: Bearer $APP_ACCESS_TOKEN --header 'Content-Type: application/json' --data-raw '{
   "type": "tweets",
   "resumable": true
}'
```

Si votre appel à l’API réussit, vous recevrez une réponse similaire à la suivante :

```
{
   "data": {
       "download_expires_at": "2021-08-18T19:42:55.000Z",
       "status": "created",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/submission/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "created_at": "2021-08-11T19:42:55.000Z",
       "resumable": false,
       "id": "1425543269983784962",
       "type": "tweets",
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/delivery/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "upload_expires_at": "2021-08-11T19:57:55.000Z"
   }
}
```

Notez la valeur du champ upload&#95;url, vous en aurez besoin dans les étapes suivantes.

#### Deuxième étape : 

Ensuite, vous devez initier le téléversement reprenable. Pour ce faire, effectuez un appel **POST** vers l’upload&#95;url de l’étape précédente et veillez à inclure les en-têtes suivants :

Content-Type: text/plain

Content-Length: 0

x-goog-resumable: start

```
curl --request POST \
'https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=' \
--header 'Content-Type: text/plain' --header \
 'Content-Length: 0' --header \
 'x-goog-resumable: start
```

Si cet appel réussit, vous recevrez un code de réponse 201. Ensuite, dans l’en-tête de réponse, copiez la valeur de l’en-tête Location, qui ressemblera à ceci :

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

Vous pouvez ensuite téléverser vos IDs de Post ou d’utilisateur à cet emplacement en suivant l’étape deux et les suivantes, [depuis le guide de démarrage rapide](/fr/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

En raison de leur complexité technique, les envois téléversables (reprises) sont à privilégier avec du code. Ce guide utilise Node.js avec la [bibliothèque de requêtes needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Installer les dépendances
</div>

Avant de continuer, vous devez disposer d’un environnement Node.js ; vous pouvez obtenir Node.js depuis son site web. Une fois installé, Node.js inclut un utilitaire appelé npm ; vérifiez que Node et npm sont bien installés en exécutant la commande suivante et assurez-vous qu’elle ne renvoie pas d’erreur.

`$ npm -v
6.4.1`

Un numéro de version similaire indique que votre environnement est prêt (notez que votre numéro de version peut varier). Nous utiliserons npm pour installer la bibliothèque d’upload. Exécutez cette commande :

`$ npm install -g needle`

Tout est prêt ; aucune configuration supplémentaire n’est nécessaire.

<div id="request-a-resumable-destination">
  #### Demander une destination reprenable
</div>

Lors de la création d’un nouveau job, définissez le paramètre resumable sur true afin d’obtenir une destination qui prend en charge les envois reprenables. Dans la réponse, vous recevrez une valeur pour upload&#95;url.

```
"upload_url":\
"https://storage.googleapis.com/compliance_tweet_ids/customer_test_object_12950882_GlYjiE?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=193969463581-compute%40developer.gserviceaccount.com%2F20200618%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20200618T184154Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature="
```

<div id="prepare-the-code-to-upload-a-file">
  #### Préparez le code pour téléverser un fichier
</div>

Par défaut, la bibliothèque crée une nouvelle destination de téléversement à partir d’un emplacement (appelé bucket) et du nom du fichier que vous souhaitez téléverser. Comme les points de terminaison de conformité par lots créent leur propre destination, nous devons indiquer à la bibliothèque que nous disposons déjà d’un emplacement prêt à recevoir notre téléversement.

Nous devrons transmettre cette valeur à la bibliothèque de téléversement, ainsi que le nom du fichier contenant les données à téléverser. Créez un fichier et nommez-le ***twitter-upload.js***. Ajoutez le code suivant :

```
const needle = require('needle');
const fs = require('fs');
const path = require('path');

const [, scriptName, filename, uploadURL] = process.argv;
if (!filename || !uploadURL) {
  console.error(`Usage: node ${path.basename(scriptName)} filename upload_url`);
  process.exit(-1);
}

async function uploadFile(file, url) {
  // rangeEnd is the index of the last byte in the file, i.e. number of bytes in file
  const rangeEnd = (await fs.promises.stat(file)).size;

  let options = {
    headers: {
      'Content-Range': `bytes */${rangeEnd}`,
    },
  };

  const response = await needle('put', url, null, options);

  switch (response.statusCode) {
    case 200:
    case 201:
      console.log('Upload complete');
      return;
    case 308:
      return resumeUpload(response, file, url);
    default:
      console.log('Got unexpected response code: ', response.statusCode);
      return;
  }
}

async function resumeUpload(response, file, url) {
  console.log('Upload not completed, resuming');
  if (response.headers.range) {
    let resumeOffset = Number(response.headers.range.split('-')[1]) + 1;

    let options = {
      headers: {
        'Content-Range': `bytes ${resumeOffset}-${rangeEnd-1}/${rangeEnd}`,
        'Content-Length': `${rangeEnd-resumeOffset}`,
      },
    };

    let readStream = fs.createReadStream(file, {start: resumeOffset});
    return needle('put', url, readStream, options);
  } else {
    console.log('Initiating upload');
    let options = {
      headers: {
        'Content-Type': 'text/plain'
      }
    };

    let readStream = fs.createReadStream(file);
    return needle('put', url, readStream, options);
  }
}

// Request resumable session URL
async function requestResumableSession(url) {
  const options = {
    headers: {
      'Content-Type': 'text/plain',
      'Content-Length': '0',
      'x-goog-resumable': 'start',
    },
  };

  const res = await needle('post', url, null, options);
  if (res.statusCode === 201) {
    const resumableSessionURL = res.headers['location'];
    console.log('Starting upload to: ', resumableSessionURL);

    await uploadFile(filename, resumableSessionURL);
  } else {
    console.log('Failed to create resumable session URI');
  }

}

requestResumableSession(uploadURL).then(result => console.log('Upload complete'));
```

Enregistrez le fichier à l’endroit le plus approprié. Ensuite, dans votre ligne de commande, exécutez le script en lui passant deux paramètres :

1. Le premier est l’emplacement du fichier (contenant les IDs de Post ou d’utilisateur) que vous souhaitez téléverser.
2. Le second est l’URL de téléversement reçue dans la réponse du point de terminaison de conformité.

Assurez-vous que l’URL est entourée de guillemets doubles, et faites de même pour le nom de votre fichier s’il contient des espaces ou d’autres caractères :

```
node twitter-upload.js compliance_upload.txt\
"https://storage.googleapis.com/compliance_tweet_ids/customer_test_object_12950882_GlYjiE?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=193969463581-compute%40developer.gserviceaccount.com%2F20200618%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20200618T184154Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature="
```

Vous verrez une sortie similaire à la suivante :

`Starting upload to:  https://storage.googleapis.com/twttr-tweet-compliance/<redacted>
Upload not completed, resuming
Initiating upload`

Vous pouvez interrompre l’envoi à tout moment en appuyant sur Ctrl + C ou en fermant votre interface en ligne de commande. Vous pourrez reprendre l’envoi là où vous vous êtes arrêté en relançant la même commande ultérieurement. Une fois le fichier entièrement envoyé, le message suivant s’affichera :

`Upload complete`

À ce stade, vous pourrez utiliser le [point de terminaison d’état de conformité](/fr/x-api/compliance/get-compliance-job) pour vérifier l’état de votre tâche de conformité, et vous pourrez télécharger le résultat de conformité une fois prêt.