...
sidebarTitle: Rechercher des publications (récentes)
---

import { Button } from '/snippets/fr/button.mdx';

<div id="getting-started-with-the-recent-post-counts-endpoint">
  ### Bien démarrer avec l’endpoint des décomptes récents de Posts
</div>

Ce guide de démarrage rapide vous aidera à effectuer votre première requête vers l’endpoint des décomptes récents de Posts à l’aide de Postman, un outil graphique qui permet d’envoyer des requêtes HTTP.
...
<Note>
  ...
  * Créer un [Project](/fr/resources/fundamentals/projects) et une [developer App](/fr/resources/fundamentals/developer-apps) associée dans le portail développeur.
  * Accéder à la page “Keys and tokens” de votre App pour générer les identifiants requis. Veillez à enregistrer tous les identifiants dans un emplacement sécurisé.
</Note>
...
Une fois la collection X API v2 chargée dans Postman, accédez à Post counts &gt; Recent Post counts request.

**Deuxième étape : authentifier votre requête**

Pour effectuer correctement une requête vers la X API, vous devez vérifier que vous disposez des autorisations nécessaires. Pour ce faire avec cet endpoint, vous devez authentifier votre requête à l’aide des méthodes d’authentification [OAuth 2.0 App-Only](/fr/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token).
...
Cette variable sera automatiquement utilisée dans l’onglet Authorization de la requête si vous avez suivi ces étapes correctement.
 
...
| `query` | from:XDevelopers | Requête à soumettre à l’endpoint Recent Post counts |

**Quatrième étape (facultatif) : spécifier la granularité de la requête**

Si vous cliquez sur le bouton « Send » après la troisième étape, vous obtiendrez la valeur par défaut des Recent Post counts : par heure sur les sept derniers jours. Si vous souhaitez obtenir les Recent Post counts par jour, vous devez ajouter le paramètre granularity avec la valeur day.
...
| :--- | :--- | :--- |
| **Key** | **Value** | **Description** |
| granularity | day | La granularité des résultats de Post counts. Valeurs possibles : day, hour ou minute |
